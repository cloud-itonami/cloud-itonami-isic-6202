(ns svcdesk.operation
  "OperationActor — one transition/publish/disclosure/dispute operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (SupportOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the ServiceGovernor (:govern) and
  ...
  actor and hands the decision to a human reviewer (support manager)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [svcdesk.llm :as llm]
            [svcdesk.policy :as policy]
            [svcdesk.phase :as phase]
               ...
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:correction? verdict) :dispute-request
                                          (:kb-embargo-imminent? verdict) :kb-embargo-imminent
                                          (:refund-commitment-imminent? verdict) :refund-commitment-imminent
                                          :else :low-confidence))
                        :phase ph
