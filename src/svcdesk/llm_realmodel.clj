(ns svcdesk.llm-realmodel
  "Real-model adapter for `svcdesk.llm`'s Advisor protocol — the piece that
  was genuinely missing before this file existed: `src/svcdesk/llm.cljc`'s
  SupportOps-LLM was a SEALED/deterministic mock (`svcdesk.llm/mock-advisor`,
  `svcdesk.llm/infer`) with no way to point the actor at an actual language
  model. `svcdesk.llm.cljc` already defines the seam this needed — a generic
  `svcdesk.llm/llm-advisor` that wraps ANY `langchain.model/ChatModel` in the
  exact same `svcdesk.llm/Advisor` protocol (`-advise`) the OperationActor
  graph (`svcdesk.operation/build`) calls, with the identical proposal shape
  (`:summary`/`:rationale`/`:cites`/`:source`/`:effect`/`:value`/
  `:confidence`) parsed via `svcdesk.llm/parse-proposal`. This file does NOT
  ...
  JVM-only (`.clj`, not `.cljc`) for the same reason `svcdesk.http`/
  `svcdesk.file-store` are: a real outbound HTTP call to an external model
  API needs a real HTTP client, and there is no portable 'open a TCP socket
  to the internet' primitive at the kotoba-wasm/clojurewasm/cljs/nbb tier
  this fleet prefers (CLAUDE.md's `.cljc`/`.kotoba` runtime-priority
  section). This is infrastructure glue over `svcdesk.llm`'s already-
  portable domain logic, not a reimplementation of it.

  ## Convention: this org's own `ITO_MODEL_*` precedent, renamed per-actor

  `orgs/gftdcojp/cloud-itonami`'s `cloud_itonami.runtime` namespace already
  established exactly this shape for this same lineage/org: env-var-driven
  `ITO_MODEL_PROVIDER`/`ITO_MODEL_URL`/`ITO_MODEL`/`ITO_MODEL_API_KEY`, a JDK
  `HttpClient`-based `:http-fn`, an OpenAI-compatible + Anthropic adapter
  (both already implemented generically in `langchain.model`, not
  reimplemented here), and a `preflight` function that reports what's
  missing without ever attempting a live call. Sibling actors
  `cloud-itonami-isic-5820` (`crm.llm-realmodel`) and `cloud-itonami-isic-
  6201` (`marketing.llm-realmodel`) already adapted that SAME shape for
  their own domains; this file closes the identical parity gap for
  `cloud-itonami-isic-6202` — deliberately not inventing an incompatible
  new one — with an `ISIC6202_`-prefixed set of env vars so multiple
  `cloud-itonami-isic-*` actors running on the same host/CI never collide
  on one another's model config:

    ISIC6202_MODEL_PROVIDER  \"openai\" (default) | \"anthropic\" | \"openclaw\"
    ...
    ISIC6202_MODEL_URL       required for \"openclaw\" (no canonical default
                             endpoint); optional override for openai/
                             anthropic (defaults to the public API URL
                             `langchain.model` already hardcodes)
    ISIC6202_MODEL           chat model name (default: gpt-4o-mini for
                             openai/openclaw, claude-opus-4-8 for anthropic)
    ISIC6202_MODEL_API_KEY   the API key/bearer token. THE SOLE TRIGGER:
                             `svcdesk.http` switches from the sealed mock
                             advisor to this real-model advisor if and only
                             if this is set and non-blank (see `svcdesk.http`'s
                             `resolve-advisor!`).
  ...
  `test/svcdesk/llm_realmodel_test.clj`): `preflight`'s missing/present
  ...
  standing in for \"the model API\" (proves the adapter builds a well-
  formed OpenAI-compatible request and parses a well-formed response — it
  does NOT prove any real target model API actually accepts this exact
...
(def providers-needing-url
  ...
  so `ISIC6202_MODEL_URL` is required for it. `:openai`/`:anthropic` both
...
(defn model-config
  ...
  then `ISIC6202_MODEL_*` env vars, then a provider-appropriate default.
  ...
  ([{:keys [provider url api-key model]}]
   (let [provider (keyword (or (nonblank provider)
                               (env "ISIC6202_MODEL_PROVIDER")
                               "openai"))
         url (or (nonblank url) (env "ISIC6202_MODEL_URL"))
         api-key (or (nonblank api-key) (env "ISIC6202_MODEL_API_KEY"))
...
(defn preflight
  ...
  `ISIC6202_MODEL_*` env var names still needed, in check order:
  ...
  ([opts]
   (let [{:keys [provider url api-key?] :as cfg} (model-config opts)
         missing (cond-> []
                   ...
                   (conj :ISIC6202_MODEL_PROVIDER)

                   (and (providers-needing-url provider) (str/blank? url))
                   (conj :ISIC6202_MODEL_URL)

                   (not api-key?)
                   (conj :ISIC6202_MODEL_API_KEY))]
...
(defn jvm-http-fn
  ...
  built-in, zero extra dependency, same client `svcdesk.http_test.clj` (and
  this file's own tests) already use to drive an HTTP server/stub."
  ([] (jvm-http-fn {}))
     ...
     (fn [{:keys [url method headers body]}]
       (let [builder (-> (HttpRequest/newBuilder (URI/create url))
             ...
             request (case method
                       ...
                       (throw (ex-info "svcdesk.llm-realmodel/jvm-http-fn: unsupported HTTP method"
                                       {:method method})))
...
(defn real-chat-model
  ...
  `ISIC6202_MODEL_*` env vars (or explicit `opts` overrides, same keys as
  `model-config`, plus `:http-fn` to inject a stub for testing — see
  `test/svcdesk/llm_realmodel_test.clj`). Delegates the actual request/
  response wire shape entirely to `langchain.model/anthropic-model`/
  `openai-model` (both already generic, already tested in
  `kotoba-lang/langchain`) — this function only resolves config and picks
  which of those two to call. Throws (does not silently fall back) for any
  provider outside `known-providers`."
  ([] (real-chat-model {}))
  ([opts]
   (let [{:keys [provider url model]} (model-config opts)
         api-key (or (:api-key opts) (env "ISIC6202_MODEL_API_KEY"))
     ...
     (case provider
       ...
       (throw (ex-info (str "svcdesk.llm-realmodel: unsupported ISIC6202_MODEL_PROVIDER " (pr-str provider)
                            " (supported: " (pr-str known-providers) ")")
                       {:provider provider :supported known-providers}))))))

(defn real-advisor
  "A real-model `svcdesk.llm/Advisor` — satisfies the EXACT SAME protocol
  `svcdesk.llm/mock-advisor` does (`-advise`, called by
  `svcdesk.operation/build`'s `:advise` node with `(store request)` and
  ...
  back. Built by wrapping `real-chat-model` in `svcdesk.llm/llm-advisor`,
  which already exists in `svcdesk.llm.cljc` for exactly this purpose
  (system prompt + fact-gathering + EDN proposal parsing all already
  implemented there, not duplicated here)."
  ([] (real-advisor {}))
